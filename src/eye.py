...
redbird = pygame.image.load("D:/python/PycharmProjects/ty/theAngryBirds/resources/images/red-bird3.png").convert_alpha()
background2 = pygame.image.load(
    "D:/python/PycharmProjects/ty/theAngryBirds/resources/images/background3.png").convert_alpha()
sling_image = pygame.image.load(
    "D:/python/PycharmProjects/ty/theAngryBirds/resources/images/sling-3.png").convert_alpha()
full_sprite = pygame.image.load(
    "D:/python/PycharmProjects/ty/theAngryBirds/resources/images/full-sprite.png").convert_alpha()
...
buttons = pygame.image.load(
    "D:/python/PycharmProjects/ty/theAngryBirds/resources/images/selected-buttons.png").convert_alpha()
pig_happy = pygame.image.load(
    "D:/python/PycharmProjects/ty/theAngryBirds/resources/images/pig_failed.png").convert_alpha()
stars = pygame.image.load(
    "D:/python/PycharmProjects/ty/theAngryBirds/resources/images/stars-edited.png").convert_alpha()
...
space = pm.Space()
space.gravity = (0.0, -700.0) #700 units per second 
...
def vector(p0, p1):#used for sling or launch
...
def unit_vector(v):
    #unit_vector is used for determining the direction of bird launch,
...
def distance(xo, yo, x, y):
    #and distance is used for collision detection in game.
...
def load_music():
    """Load the music"""
    song1 = 'D:/python/PycharmProjects/ty/theAngryBirds/resources/sounds/angry-birds.ogg'
...
def game():
    global running, wall, mouse_pressed, sling_x, sling_y, sling2_x, sling2_y
    global mouse_distance, rope_lenght, t1, t2, x_mouse, y_mouse
    global bird_path, birds, pigs, static_lines, static_lines1, columns, beams
    global counter, restart_counter, score, game_state, bonus_score_once
    while running:
        # Input handling
        ...
        pygame.display.flip()
        clock.tick(50)
        pygame.display.set_caption("The Angry Bird")
    ...
    while True:
        ...
        if landmark_points:
            ...
            if (left[0].y - left[1].y) < 0.004:
                if click_count == 0:
                    click_count += 1
                elif click_count == 1 and not mouse_held:
                    pyautogui.mouseDown()
                    mouse_held = True
                    print("mouse hold")
            else:
                if click_count == 1 and mouse_held:
                    pyautogui.mouseUp()
                    mouse_held = False
                    click_count = 0
...
if __name__ == "__main__":
    ...
    import threading
    game_thread = threading.Thread(target=eye_reco)
    game_thread.start()
